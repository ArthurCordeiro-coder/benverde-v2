...
const LUMII_SYSTEM_PROMPT = `Voce e a Lumii, gerente de dados inteligente da LUMII, uma distribuidora de bananas e hortifruti.
...
function shouldIncludeScope(
  ...
  if (
    ...
    requestedScope === "lumii-ia" ||
    requestedScope === "mita-ai"
  ) {
    return true;
  }
...
function logLumiiContextError(section: string, error: unknown): void {
  console.error(`Falha ao montar o contexto da Lumii para ${section}.`, error);
}

async function buildLumiiContext(scope: DashboardScope): Promise<string> {
  ...
  if (shouldIncludeScope(scope, "estoque")) {
    try {
      ...
      logLumiiContextError("estoque", error);
    }
  }
  ...
  if (shouldIncludeScope(scope, "precos")) {
    try {
      ...
      logLumiiContextError("precos", error);
    }
  }
  ...
  if (shouldIncludeScope(scope, "caixas")) {
    try {
      ...
      logLumiiContextError("caixas", error);
    }
  }
  ...
  if (shouldIncludeScope(scope, "metas")) {
    try {
      ...
      logLumiiContextError("metas", error);
    }
  }
...
export async function chatWithLumii(payload: unknown, scope: DashboardScope): Promise<{
  answer: string;
  history: ChatMessage[];
}> {
  ...
  const messages = [
    {
      role: "system",
      content: `${LUMII_SYSTEM_PROMPT}\n\n${await buildLumiiContext(scope)}`,
    },
    ...history,
