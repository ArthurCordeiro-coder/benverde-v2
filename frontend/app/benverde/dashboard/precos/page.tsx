      ...
      <section className="space-y-6 pb-8 animate-in fade-in duration-500">
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto]">
          ...
          <button
            ...
            className={`flex items-center gap-3 rounded-2xl border px-6 py-4 text-sm font-bold transition-all ${onlyBananas
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-[#0f5922] text-white">
                <tr>
                  ...
                  {markets.map((market) => (
                    <th key={market} className="border-r border-white/5 p-5 text-right text-[10px] font-bold uppercase tracking-widest">
                      <button
                        type="button"
                        onClick={() =>
                          setSortConfig((current) => ({
              ...
              <tbody className="divide-y divide-white/5">
                {isLoading ? (
                  <tr>
                    <td colSpan={4 + markets.length} className="p-5 text-gray-400">
                      Carregando preços...
                    </td>
                  </tr>
                  ...
                  visibleRows.map((row) => {
                    ...
                    return (
                      <tr
                        ...
                        className={`group transition-colors ${isBanana
        ...
        <div className="fixed bottom-24 right-4 z-[100] flex h-[70vh] max-h-[550px] w-[calc(100vw-2rem)] max-w-[400px] flex-col overflow-hidden rounded-[32px] border border-white/15 bg-[#07130d]/95 shadow-2xl backdrop-blur-2xl animate-in slide-in-from-bottom-8 duration-300 lg:bottom-28 lg:right-8 lg:h-[550px] lg:w-[400px]">
          <header className="flex items-center justify-between border-b border-white/10 bg-gradient-to-r from-emerald-500/10 to-transparent p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-sm font-bold text-white">Lumii Inteligência</h2>
                    ...
                    {message.role === "assistant" ? (
                      <ReactMarkdown
                        ...
                        components={{
                          ...
                          em: ({ children }) => <em className="italic text-emerald-200/80">{children}</em>,
                          ul: ({ children }) => <ul className="mb-2 ml-4 list-disc space-y-1">{children}</ul>,
                          ol: ({ children }) => <ol className="mb-2 ml-4 list-decimal space-y-1">{children}</ol>,
                          li: ({ children }) => <li className="leading-relaxed">{children}</li>,
                          code: ({ children, className }) =>
                            className?.includes("language-") ? (
                              <code className="block overflow-x-auto rounded-lg bg-black/30 px-3 py-2 font-mono text-xs text-emerald-200">{children}</code>
                            ) : (
                              <code className="rounded bg-black/30 px-1 py-0.5 font-mono text-xs text-emerald-300">{children}</code>
                            ),
                          pre: ({ children }) => <pre className="mb-2 whitespace-pre-wrap">{children}</pre>,
                          h1: ({ children }) => <h1 className="mb-2 text-base font-bold text-emerald-200">{children}</h1>,
                          h2: ({ children }) => <h2 className="mb-1 text-sm font-bold text-emerald-200">{children}</h2>,
                          h3: ({ children }) => <h3 className="mb-1 text-sm font-semibold text-emerald-200/80">{children}</h3>,
                          blockquote: ({ children }) => <blockquote className="mb-2 border-l-2 border-emerald-400/40 pl-3 italic text-emerald-200/70">{children}</blockquote>,
            ...
            {isLumiiTyping ? (
              <div className="flex justify-start animate-pulse">
                <div className="flex items-center gap-2 rounded-2xl border border-emerald-400/10 bg-emerald-500/5 px-4 py-3 text-xs font-medium italic text-emerald-200">
                  ...
                  Lumii está processando os dados...
                </div>
              </div>
            ) : null}
