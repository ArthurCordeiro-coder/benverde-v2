...
export default function ArquivosPage() {
  ...
  const onPreview = (f: DisplayFile) => setPreview(f);
    ...
    <div className="text-gray-300 selection:bg-emerald-500/30">
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
        <div className="flex items-center gap-4 flex-wrap">
          <button
            onClick={() => showToast("Envio será habilitado em breve.")}
            className="flex items-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-black px-5 py-2.5 rounded-xl font-bold text-sm transition-all shadow-lg shadow-emerald-500/10"
          >
            <Upload size={18} />
            Enviar arquivo
          </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-6 gap-4">
            ...
            {visible.map((file) => (
              ...
              >
                ...
                <div
                  onClick={(e) => e.stopPropagation()}
                  className="absolute inset-0 bg-[#0d100f]/80 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-opacity rounded-xl flex items-center justify-center gap-3"
                >
                  {file.type === "folder" ? (
                    <button
                      onClick={() => onItemClick(file)}
                      className="w-10 h-10 rounded-full bg-emerald-500 text-black flex items-center justify-center hover:scale-110 transition-transform shadow-lg shadow-emerald-500/20"
                    >
                      <Folder size={18} />
                    </button>
                  ) : (
      ...
      {/* FAB */}
      <button
        onClick={() => showToast("Envio será habilitado em breve.", "#10b981")}
        className="fixed bottom-6 right-6 w-14 h-14 bg-emerald-500 rounded-2xl flex items-center justify-center text-black shadow-2xl shadow-emerald-500/40 hover:scale-105 active:scale-95 transition-all z-20"
      >
        <Upload size={24} />
      </button>
