...
export type MitaPdfItem = LumiiPdfItem;
...
function normalizeLumiiItems(payload: unknown): LumiiPdfItem[] {
  ...
  for (const rawItem of rawItems) {
    ...
    const produto = String(
      item.produto ?? item.item ?? item.descricao ?? item.descricao ?? "",
    )
      .trim()
      .toUpperCase();
...
async function uploadFileToXai(file: File, apiKey: string): Promise<string> {
  ...
  if (!response.ok || !payload.id) {
    const detail =
      payload?.error?.message?.trim() || "Nao foi possivel enviar o PDF para a LUMII-IA.";
...
async function deleteFileFromXai(fileId: string, apiKey: string): Promise<void> {
  try {
    ...
    console.error(`Falha ao remover arquivo temporario ${fileId} da LUMII-IA.`);
  }
}
    ...
    if (!response.ok) {
      const detail =
        ...
        payload.error.message.trim()
          ...
          : "Nao foi possivel processar o PDF com a LUMII-IA.";
      throw new HttpError(502, detail);
    }
